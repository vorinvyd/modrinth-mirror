...
export default function Garland() {
  ...
  useEffect(() => {
    ...
    function calculateItemWidth() {
      if (cachedItemWidth > 0) return cachedItemWidth
      let totalWidth = 0
      ...
      if (validMeasurements > 0) {
        cachedItemWidth = totalWidth / validMeasurements
      } else {
        const containerWidth = garlandContainer.scrollWidth || garlandContainer.offsetWidth
        cachedItemWidth = containerWidth > 0 ? containerWidth / originalItems.length : 0
      }
  ...
  const GarlandItem = ({ itemIndex }) => {
    const uniqueId = `garland-${itemIndex}`
    return (
